        ...
        <div className="grid lg:grid-cols-2 gap-12">
          {coffeeMenu.map((category, categoryIndex) => (
            <div key={categoryIndex} className="bg-coffee-50 rounded-2xl p-8">
              ...
              <div className="space-y-6" role="list" aria-label={`${category.category} 메뉴`}>
                {category.items.map((item, itemIndex) => (
                  <div key={itemIndex} className="flex justify-between items-start group hover:bg-white hover:shadow-sm rounded-lg p-4 transition-all duration-200" role="listitem">
                    ...
                    <div className="ml-4 flex-shrink-0">
                      <span className="text-lg font-bold text-brown-700" aria-label={`가격: ${item.price}원`}>
                        {item.price}원
                      </span>
                    </div>
                  </div>
        ...
        <div className="mt-16 bg-gradient-to-r from-brown-700 to-brown-800 rounded-2xl p-8 text-center text-white">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            <div className="flex items-center space-x-2">
              ...
              <a href="mailto:bfield20@daum.net" className="hover:underline" aria-label="원두 납품 문의 이메일: bfield20@daum.net">
                bfield20@daum.net
              </a>
            </div>
            <div className="text-coffee-200" aria-hidden="true">|</div>
            <div className="flex items-center space-x-2">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        ...
        <div className="mt-12 text-center">
          <div className="inline-flex items-center space-x-2 bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg transition-colors duration-200">
            ...
            <a 
              ...
              aria-label="네이버 스마트스토어에서 주문하기 (새 창에서 열림)"
            >
              네이버 스마트스토어에서 주문하기
            </a>
          </div>
